    ...
    <Card className="relative overflow-hidden bg-black w-[380px] h-[670px] mx-auto rounded-xl shadow-2xl">
      <CardContent className="p-0 aspect-[9/16] relative">
        ...
        <div className="absolute right-4 bottom-32 flex flex-col items-center gap-6">
          {[
            {
              ...
              label: formatNumber(likes + (isLiked ? 1 : 0)),
              onClick: () => setIsLiked(!isLiked),
              class: isLiked ? 'animate-bounce' : ''
            },
            {
              icon: <MessageCircle className="h-7 w-7 text-white" />,
              label: formatNumber(comments + localComments.length),
              onClick: () => {
                if (onCommentClick) {
                  onCommentClick();
                } else {
          ...
          {/* Left Info */}
          <div className="absolute bottom-24 left-4 space-y-4 w-72">
            {/* Audio Info */}
            <motion.div 
              className="flex items-center gap-3 bg-black/40 backdrop-blur-sm rounded-full p-2 hover:bg-black/60 cursor-pointer"
